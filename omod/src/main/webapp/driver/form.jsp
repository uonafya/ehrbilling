...
<openmrs:require privilege="Add/Edit Driver" otherwise="/login.htm"
	redirect="/module/ehrbilling/main.form" />

<%@ include file="/WEB-INF/template/header.jsp"%>
<h2>
	<spring:message code="ehrbilling.driver.manage" />
</h2>
...
<form method="post" class="box">
	<table>
		<tr>
			...
			<td><spring:bind path="driver.name">
		...
		<tr>
			...
			<td><spring:bind path="driver.description">
					<input type="text" name="${status.expression}"
						value="${status.value}" size="35" />
					<c:if test="${status.errorMessage != ''}">
						<span class="error">${status.errorMessage}</span>
		...
		<tr>
			...
			<td><spring:bind path="driver.phone">
					<input type="text" name="${status.expression}"
						value="${status.value}" size="35" />
					<c:if test="${status.errorMessage != ''}">
						<span class="error">${status.errorMessage}</span>
		...
		<tr>
			...
			<td><spring:bind path="driver.address">
					<input type="text" name="${status.expression}"
						value="${status.value}" size="35" />
					<c:if test="${status.errorMessage != ''}">
						<span class="error">${status.errorMessage}</span>
		...
		<tr>
			...
			<td><spring:bind path="driver.retired">
	...
	<br /> <input type="submit"
		...
		onclick="javascript:window.location.href='driver.list'">
</form>
